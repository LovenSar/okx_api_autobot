...
def analyze_with_deepseek(price_data):
    ...
    - 技术微结构与动量：20%
    - 成交量与流动性：30%
    - 情绪与其他快速指标（RSI/MACD/BOLL回归）：10%（仅调节信心，不独立触发，一般别人恐惧我贪婪，别人贪婪我恐惧）
    - 用梅花易进行概率预测，作为参考，用易经推理，给出概率，作为参考。
    ...
    - 信号共振：如果多个信号共振，则认为信号有效，否则认为信号待定，需要进一步确认。
    ...
    【分析要求】
    请依据以上流程与约束，综合趋势、关键位、动量与量能，并结合情绪作信心校正，根据当前持仓情况给出明确交易指令。

    请用以下 JSON 格式回复：
    {{
        "signal": "BUY|SELL|HOLD",
        "reason": "必须包含4H趋势 + 15m择时与触发细节，说明量能/情绪是否同向，并说明为什么做出这个决定，建议做多还是做空，可以尝试超短线还是中长线。",
        "stop_loss": 具体价格,
        "take_profit": 具体价格,
