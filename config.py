...
# 后端主循环（AI 决策调度）间隔（秒）
BACKEND_DECISION_INTERVAL_SECONDS = _get_float("BACKEND_DECISION_INTERVAL_SEC", 600)
...
TRADE_MIN_INTERVAL_SECONDS = _get_int("MIN_TRADE_INTERVAL_SEC", 600)

# 私有接口（余额/持仓）刷新节流（秒）
PRIVATE_UPDATE_INTERVAL_SECONDS = _get_float("PRIVATE_UPDATE_INTERVAL_SEC", 1)

# 技术分析整体刷新间隔（秒）（默认15分钟）
ANALYSIS_UPDATE_INTERVAL_SECONDS = _get_float("ANALYSIS_UPDATE_INTERVAL_SEC", 600)

# 情绪数据缓存 TTL（秒）（默认15分钟）
SENTIMENT_CACHE_TTL_SECONDS = _get_float("SENTIMENT_TTL_SEC", 600)

# AI 决策最小调用间隔（秒）（默认100秒）
AI_DECISION_MIN_INTERVAL_SECONDS = _get_float("AI_DECISION_INTERVAL_SEC", 600)

# 反转下单所需的连续同向确认次数（含当前信号）
REVERSAL_CONFIRMATION_COUNT = _get_int("REVERSAL_CONFIRMATION_COUNT", 3)
...
BREAKOUT_COOLDOWN_SEC = _get_int("BREAKOUT_COOLDOWN_SEC", 600)

# 连涨加仓策略（同向加仓）
# 是否启用同向加仓（1启用，0关闭）
PYRAMID_ENABLED = _get_int("PYRAMID_ENABLED", 1)
# 同向加仓的最小间隔（秒），独立于最小交易间隔
PYRAMID_MIN_INTERVAL_SEC = _get_int("PYRAMID_MIN_INTERVAL_SEC", 600)
# 同向加仓的最大次数（每个方向）
