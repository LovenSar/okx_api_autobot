...
# 后端主循环（AI 决策调度）间隔（秒）
BACKEND_DECISION_INTERVAL_SECONDS = _get_float("BACKEND_DECISION_INTERVAL_SEC", 180)

# 后端实时数据刷新线程间隔（秒）
BACKEND_REALTIME_UPDATE_INTERVAL_SECONDS = _get_float("REALTIME_UPDATE_INTERVAL_SEC", 60)

# 前端页面自动刷新间隔（毫秒）
FRONTEND_REFRESH_INTERVAL_MS = _get_int("FRONTEND_REFRESH_INTERVAL_MS", 1000)

# 交易最小间隔（秒），用于防频繁交易（默认15分钟）
TRADE_MIN_INTERVAL_SECONDS = _get_int("MIN_TRADE_INTERVAL_SEC", 180)

# 私有接口（余额/持仓）刷新节流（秒）
PRIVATE_UPDATE_INTERVAL_SECONDS = _get_float("PRIVATE_UPDATE_INTERVAL_SEC", 60)

# 技术分析整体刷新间隔（秒）（默认15分钟）
ANALYSIS_UPDATE_INTERVAL_SECONDS = _get_float("ANALYSIS_UPDATE_INTERVAL_SEC", 180)

# 情绪数据缓存 TTL（秒）（默认15分钟）
SENTIMENT_CACHE_TTL_SECONDS = _get_float("SENTIMENT_TTL_SEC", 180)

# AI 决策最小调用间隔（秒）（默认100秒）
AI_DECISION_MIN_INTERVAL_SECONDS = _get_float("AI_DECISION_INTERVAL_SEC", 180)
...
BREAKOUT_COOLDOWN_SEC = _get_int("BREAKOUT_COOLDOWN_SEC", 180)

# 连涨加仓策略（同向加仓）
# 是否启用同向加仓（1启用，0关闭）
PYRAMID_ENABLED = _get_int("PYRAMID_ENABLED", 1)
# 同向加仓的最小间隔（秒），独立于最小交易间隔
PYRAMID_MIN_INTERVAL_SEC = _get_int("PYRAMID_MIN_INTERVAL_SEC", 180)
# 同向加仓的最大次数（每个方向）
